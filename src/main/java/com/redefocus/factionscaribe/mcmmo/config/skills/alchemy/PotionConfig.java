package com.redefocus.factionscaribe.mcmmo.config.skills.alchemy;

import com.redefocus.factionscaribe.mcmmo.config.ConfigLoader;
import com.redefocus.factionscaribe.mcmmo.datatypes.skills.alchemy.AlchemyPotion;
...
public class PotionConfig extends ConfigLoader {
    ...
    /**
     * Parse a ConfigurationSection representing a AlchemyPotion. Returns null
    ...
    private AlchemyPotion loadPotion(ConfigurationSection potion_section) {
        try {
            ...
            if (potion_section.contains("Effects")) {
                for (String effect : potion_section.getStringList("Effects")) {
                    ...
                    if (type != null) {
                        ...
                        FactionsCaribe.getInstance().getLogger().warning("Failed to parse effect for potion " + name + ": " + effect);
                    }
                }
            }
            ...
            if (potion_section.contains("Children")) {
                for (String child : potion_section.getConfigurationSection("Children").getKeys(false)) {
                    ...
                    if (ingredient != null) {
                        ...
                        FactionsCaribe.getInstance().getLogger().warning("Failed to parse child for potion " + name + ": " + child);
                    }
                }
            }
            ...
            FactionsCaribe.getInstance().getLogger().warning("Failed to load Alchemy potion: " + potion_section.getName());
            return null;
        }
    }

    /**
     * Parse a string representation of an ingredient. Format:
     * '&lt;MATERIAL&gt;[:data]' Returns null if input cannot be parsed.
     *
