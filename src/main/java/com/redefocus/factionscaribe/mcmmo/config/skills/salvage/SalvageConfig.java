package com.redefocus.factionscaribe.mcmmo.config.skills.salvage;
...
import com.redefocus.factionscaribe.mcmmo.skills.salvage.salvageables.SalvageableFactory;
...
public class SalvageConfig extends ConfigLoader {
    ...
    private boolean noErrorsInSalvageable(List<String> issues) {
        if (!issues.isEmpty()) {
            FactionsCaribe.getInstance().getLogger().warning("Errors have been found in: " + fileName);
            FactionsCaribe.getInstance().getLogger().warning("The following issues were found:");
        }
