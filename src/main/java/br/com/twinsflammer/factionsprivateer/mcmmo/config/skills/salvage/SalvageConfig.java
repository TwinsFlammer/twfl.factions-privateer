...
public class SalvageConfig extends ConfigLoader {
    ...
    private boolean noErrorsInSalvageable(List<String> issues) {
        if (!issues.isEmpty()) {
            FactionsPrivateer.getInstance().getLogger().warning("Errors have been found in: " + fileName);
            FactionsPrivateer.getInstance().getLogger().warning("The following issues were found:");
        }
