...
import br.com.twinsflammer.factionsprivateer.mcmmo.skills.swords.Swords;
...
public final class CombatUtils {
    ...
    private static void startGainXp(McMMOPlayer mcMMOPlayer, LivingEntity target, SkillType skillType, double multiplier) {
        ...
        if (baseXP != 0) {
            new AwardCombatXpTask(mcMMOPlayer, skillType, baseXP, target, xpGainReason).runTaskLater(FactionsPrivateer.getInstance(), 0);
        }
    }
    ...
    public static double callFakeDamageEvent(Entity attacker, Entity target, DamageCause damageCause, double damage) {
        ...
        FactionsPrivateer.getInstance().getServer().getPluginManager().callEvent(damageEvent);
