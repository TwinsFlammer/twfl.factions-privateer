...
import br.com.twinsflammer.factionsprivateer.mcmmo.events.fake.FakeBrewEvent;
...
//                System.out.println("finishBrewing stage " + potionStage.name() + " | " + UserManager.hasPlayerDataKey(player));
    ...
    public static void scheduleCheck(Player player, BrewingStand brewingStand) {
        new AlchemyBrewCheckTask(player, brewingStand).runTask(FactionsPrivateer.getInstance());
    }

    public static void scheduleUpdate(Inventory inventory) {
        for (HumanEntity humanEntity : inventory.getViewers()) {
            if (humanEntity instanceof Player) {
                new PlayerUpdateInventoryTask((Player) humanEntity).runTask(FactionsPrivateer.getInstance());
            }
        }
    }
}
