...
import br.com.twinsflammer.factionsprivateer.mcmmo.datatypes.skills.ModConfigType;
...
public class McImportCommand implements CommandExecutor {
    ...
    public boolean importModConfig() {
        ...
        FactionsPrivateer.getInstance().getLogger().info("Starting import of mod materials...");
        fileAmount = 0;
        ...
        try {
            ...
            FactionsPrivateer.getInstance().getLogger().warning("Could not find " + importFile.getAbsolutePath() + " ! (No such file or directory)");
            FactionsPrivateer.getInstance().getLogger().warning("Copy and paste latest.log to " + importFile.getParentFile().getAbsolutePath() + " and rename it to import.log");
            return false;
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        ...
        FactionsPrivateer.getInstance().getLogger().info("Import finished! Created " + fileAmount + " files!");
        return true;
    }
    ...
    private void createOutput(ModConfigType modConfigType, HashMap<String, ArrayList<String>> materialNames) {
        ...
        if (!outputFilePath.exists() && !outputFilePath.mkdirs()) {
            FactionsPrivateer.getInstance().getLogger().severe("Could not create output directory! " + outputFilePath.getAbsolutePath());
        }
        ...
        for (String modName : materialNames.keySet()) {
            ...
            FactionsPrivateer.getInstance().getLogger().info("Creating " + outputFile.getName());
            try {
                if (outputFile.exists() && !outputFile.delete()) {
                    FactionsPrivateer.getInstance().getLogger().severe("Not able to delete old output file! " + outputFile.getAbsolutePath());
                }

                if (!outputFile.createNewFile()) {
                    FactionsPrivateer.getInstance().getLogger().severe("Could not create output file! " + outputFile.getAbsolutePath());
                    continue;
                }
                ...
                if (configSections == null) {
                    FactionsPrivateer.getInstance().getLogger().severe("Something went wrong!! type is " + type);
                    return;
                }
