...
import br.com.twinsflammer.factionsprivateer.mcmmo.events.skills.fishing.McMMOPlayerShakeEvent;
...
public class FishingManager extends SkillManager {
    ...
    private boolean unleashTheKraken(boolean forceSpawn) {
        ...
        if (globalEffectsEnabled) {
            ...
            FactionsPrivateer.getInstance().getServer().broadcastMessage(ChatColor.RED + AdvancedConfig.getInstance().getServerUnleashMessage().replace("(PLAYER)", player.getDisplayName()));
        } else {
        ...
        if (!kraken.isValid()) {
            ...
            new KrakenAttackTask(kraken, player, player.getLocation()).runTaskTimer(FactionsPrivateer.getInstance(), attackInterval, attackInterval);
        ...
        int attackInterval = AdvancedConfig.getInstance().getKrakenAttackInterval() * Misc.TICK_CONVERSION_FACTOR;
        new KrakenAttackTask(kraken, player).runTaskTimer(FactionsPrivateer.getInstance(), attackInterval, attackInterval);
    ...
    public void shakeCheck(LivingEntity target) {
        ...
        FactionsPrivateer.getInstance().getServer().getPluginManager().callEvent(activationEvent);
