...
import br.com.twinsflammer.factionscaribe.mcmmo.datatypes.skills.ModConfigType;
...
public class McImportCommand implements CommandExecutor {
    ...
    public boolean importModConfig() {
        ...
        FactionsCaribe.getInstance().getLogger().info("Starting import of mod materials...");
        fileAmount = 0;
        ...
        try {
            ...
            FactionsCaribe.getInstance().getLogger().warning("Could not find " + importFile.getAbsolutePath() + " ! (No such file or directory)");
            FactionsCaribe.getInstance().getLogger().warning("Copy and paste latest.log to " + importFile.getParentFile().getAbsolutePath() + " and rename it to import.log");
            return false;
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        ...
        FactionsCaribe.getInstance().getLogger().info("Import finished! Created " + fileAmount + " files!");
        return true;
    }
    ...
    private void createOutput(ModConfigType modConfigType, HashMap<String, ArrayList<String>> materialNames) {
        ...
        if (!outputFilePath.exists() && !outputFilePath.mkdirs()) {
            FactionsCaribe.getInstance().getLogger().severe("Could not create output directory! " + outputFilePath.getAbsolutePath());
        }
        ...
        for (String modName : materialNames.keySet()) {
            ...
            FactionsCaribe.getInstance().getLogger().info("Creating " + outputFile.getName());
            try {
                if (outputFile.exists() && !outputFile.delete()) {
                    FactionsCaribe.getInstance().getLogger().severe("Not able to delete old output file! " + outputFile.getAbsolutePath());
                }

                if (!outputFile.createNewFile()) {
                    FactionsCaribe.getInstance().getLogger().severe("Could not create output file! " + outputFile.getAbsolutePath());
                    continue;
                }
                ...
                if (configSections == null) {
                    FactionsCaribe.getInstance().getLogger().severe("Something went wrong!! type is " + type);
                    return;
                }
