...
import br.com.twinsflammer.factionscaribe.mcmmo.runnables.commands.McScoreboardKeepTask;
...
public class PlayerProfileLoadingTask extends BukkitRunnable {
    ...
    public void run() {
        ...
        if (!player.isOnline()) {
            FactionsCaribe.getInstance().getLogger().info("Aborting profile loading recovery for " + player.getName() + " - player logged out");
            return;
        }
        ...
        if (profile.isLoaded()) {
            new ApplySuccessfulProfile(new McMMOPlayer(player, profile)).runTask(FactionsCaribe.getInstance());
            return;
        }

        // If we've failed five times, give up
        if (attempt >= MAX_TRIES) {
            FactionsCaribe.getInstance().getLogger().severe("Giving up on attempting to load the PlayerProfile for " + player.getName());
            FactionsCaribe.getInstance().getServer().broadcast(LocaleLoader.getString("Profile.Loading.AdminFailureNotice", player.getName()), "TESTE");
            player.sendMessage(LocaleLoader.getString("Profile.Loading.Failure").split("\n"));
            return;
        }
        new PlayerProfileLoadingTask(player, attempt).runTaskLaterAsynchronously(FactionsCaribe.getInstance(), 100 * attempt);
    }
    ...
    private class ApplySuccessfulProfile extends BukkitRunnable {
        ...
        public void run() {
            ...
            if (Config.getInstance().getShowStatsAfterLogin()) {
                ...
                new McScoreboardKeepTask(player).runTaskLater(FactionsCaribe.getInstance(), 1 * Misc.TICK_CONVERSION_FACTOR);
            }
        }
    }
}
