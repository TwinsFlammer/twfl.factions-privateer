...
public final class FlatfileDatabaseManager implements DatabaseManager {
    ...
    protected FlatfileDatabaseManager() {
        ...
        if (mcMMO.getUpgradeManager().shouldUpgrade(UpgradeType.ADD_UUIDS)) {
            new UUIDUpdateAsyncTask(mcMMO.p, getStoredUsers()).runTaskAsynchronously(FactionsCaribe.getInstance());
        }
    }
    ...
    public void purgePowerlessUsers() {
        ...
        synchronized (fileWritingLock) {
            try {
                ...
                FactionsCaribe.getInstance().getLogger().severe("Exception while reading " + usersFilePath + " (Are you sure you formatted it correctly?)" + e.toString());
            } finally {
                if (in != null) {
                    try {
                        in.close();
        ...
        FactionsCaribe.getInstance().getLogger().info("Purged " + purgedUsers + " users from the database.");
    }
    ...
    public void purgeOldUsers() {
        ...
        FactionsCaribe.getInstance().getLogger().info("Purging old users...");
        ...
        synchronized (fileWritingLock) {
            try {
                ...
                while ((line = in.readLine()) != null) {
                    ...
                    if (lastPlayed == 0) {
                        OfflinePlayer player = FactionsCaribe.getInstance().getServer().getOfflinePlayer(name);
        ...
        FactionsCaribe.getInstance().getLogger().info("Purged " + removedPlayers + " users from the database.");
    }
            ...
            try {
                ...
                while ((line = in.readLine()) != null) {
                    ...
                    if (!worked && line.split(":")[USERNAME].equalsIgnoreCase(playerName)) {
                        FactionsCaribe.getInstance().getLogger().info("User found, removing...");
                        worked = true;
    ...
    public boolean saveUserUUID(String userName, UUID uuid) {
        ...
        synchronized (fileWritingLock) {
            try {
                ...
                while ((line = in.readLine()) != null) {
                    ...
                    if (!worked && character[USERNAME].equalsIgnoreCase(userName)) {
                        if (character.length < 42) {
                            FactionsCaribe.getInstance().getLogger().severe("Could not update UUID for " + userName + "!");
                            FactionsCaribe.getInstance().getLogger().severe("Database entry is invalid.");
                            continue;
                        }
                ...
                FactionsCaribe.getInstance().getLogger().info(i + " entries written while saving UUID for " + userName);
                if (in != null) {
                    try {
                        in.close();
                    } catch (IOException e) {
            ...
            try {
                ...
                while (((line = in.readLine()) != null)) {
                    ...
                    if (!fetchedUUIDs.isEmpty() && fetchedUUIDs.containsKey(character[USERNAME])) {
                        if (character.length < 42) {
                            FactionsCaribe.getInstance().getLogger().severe("Could not update UUID for " + character[USERNAME] + "!");
                            FactionsCaribe.getInstance().getLogger().severe("Database entry is invalid.");
                            continue;
                        }
                ...
                FactionsCaribe.getInstance().getLogger().info(i + " entries written while saving UUID batch");
                if (in != null) {
                    try {
                        in.close();
                    } catch (IOException e) {
    ...
    private void updateLeaderboards() {
        ...
        synchronized (fileWritingLock) {
            try {
                ...
                FactionsCaribe.getInstance().getLogger().severe("Exception while reading " + usersFilePath + " during user " + playerName + " (Are you sure you formatted it correctly?) " + e.toString());
            } finally {
                if (in != null) {
                    try {
                        in.close();
                ...
                try {
                    ...
                    while ((line = in.readLine()) != null) {
                        ...
                        if (character.length < 33) {
                            ...
                            FactionsCaribe.getInstance().getLogger().warning("Dropping malformed or before version 1.0 line from database - " + line);
                            continue;
                        }
                        ...
                        if (Config.getInstance().getTruncateSkills()) {
                            for (SkillType skill : SkillType.NON_CHILD_SKILLS()) {
                                ...
                                if (Integer.valueOf(character[index]) > cap) {
                                    FactionsCaribe.getInstance().getLogger().warning("Truncating " + skill.getName() + " to configured max level for player " + character[USERNAME]);
                                    character[index] = cap + "";
                                    updated = true;
                                }
                            }
