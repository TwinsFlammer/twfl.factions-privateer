...
import br.com.twinsflammer.factionscaribe.mcmmo.skills.swords.Swords;
...
import br.com.twinsflammer.factionscaribe.mcmmo.runnables.skills.AwardCombatXpTask;
...
public final class CombatUtils {
    ...
    private static void startGainXp(McMMOPlayer mcMMOPlayer, LivingEntity target, SkillType skillType, double multiplier) {
        ...
        if (baseXP != 0) {
            new AwardCombatXpTask(mcMMOPlayer, skillType, baseXP, target, xpGainReason).runTaskLater(FactionsCaribe.getInstance(), 0);
        }
    }
    ...
    public static double callFakeDamageEvent(Entity attacker, Entity target, DamageCause damageCause, double damage) {
        ...
        FactionsCaribe.getInstance().getServer().getPluginManager().callEvent(damageEvent);
