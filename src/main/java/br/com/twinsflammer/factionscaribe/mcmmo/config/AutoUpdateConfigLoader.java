...
public abstract class AutoUpdateConfigLoader extends ConfigLoader {
    ...
    protected void loadFile() {
        super.loadFile();
        FileConfiguration internalConfig = YamlConfiguration.loadConfiguration(FactionsCaribe.getInstance().getResource(fileName));
        ...
        if (needSave) {
            ...
            try {
                // Read internal
                BufferedReader reader = new BufferedReader(new InputStreamReader(FactionsCaribe.getInstance().getResource(fileName)));
            ...
            try {
                ...
                if (!FactionsCaribe.getInstance().getConfig().getBoolean("General.Config_Update_Overwrite", true)) {
                    saveName += ".new";
                }

                BufferedWriter writer = new BufferedWriter(new FileWriter(new File(FactionsCaribe.getInstance().getDataFolder(), saveName)));
                writer.write(output);
                writer.flush();
                writer.close();
            } catch (Exception e) {
