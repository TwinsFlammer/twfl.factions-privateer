...
import br.com.twinsflammer.factionscaribe.mcmmo.events.items.McMMOItemSpawnEvent;
...
public final class Misc {
    ...
    public static boolean isNPCEntity(Entity entity) {
        return (entity == null || entity.hasMetadata("NPC") || entity instanceof NPC || entity.getClass().getName().equalsIgnoreCase("cofh.entity.PlayerFake")
                || ((entity instanceof ArmorStand) && !((ArmorStand) entity).isVisible()));
    }
    ...
    public static void printProgress(int convertedUsers, int progressInterval, long startMillis) {
        if ((convertedUsers % progressInterval) == 0) {
            FactionsCaribe.getInstance().getLogger().info(String.format("Conversion progress: %d users at %.2f users/second", convertedUsers, convertedUsers / (double) ((System.currentTimeMillis() - startMillis) / TIME_CONVERSION_FACTOR)));
        }
    }
