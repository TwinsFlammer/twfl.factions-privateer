...
import br.com.twinsflammer.factionscaribe.mcmmo.runnables.MobHealthDisplayUpdaterTask;
...
public final class MobHealthbarUtils {
    ...
    public static void handleMobHealthbars(Player player, LivingEntity target, double damage) {
        ...
        if (displayTime != -1) {
            ...
            if (updateName) {
                target.setMetadata(mcMMO.customNameKey, new FixedMetadataValue(FactionsCaribe.getInstance(), oldName));
                target.setMetadata(mcMMO.customVisibleKey, new FixedMetadataValue(FactionsCaribe.getInstance(), oldNameVisible));
            } else if (!target.hasMetadata(mcMMO.customNameKey)) {
                target.setMetadata(mcMMO.customNameKey, new FixedMetadataValue(FactionsCaribe.getInstance(), ""));
                target.setMetadata(mcMMO.customVisibleKey, new FixedMetadataValue(FactionsCaribe.getInstance(), false));
            }

            new MobHealthDisplayUpdaterTask(target).runTaskLater(FactionsCaribe.getInstance(), displayTime * Misc.TICK_CONVERSION_FACTOR); // Clear health display after 3 seconds
        }
    }
