package br.com.twinsflammer.factionscaribe.mcmmo.util.scoreboards;
...
public class ScoreboardWrapper {
    ...
    public static ScoreboardWrapper create(Player player) {
        return new ScoreboardWrapper(player.getName(), FactionsCaribe.getInstance().getServer().getScoreboardManager().getNewScoreboard());
    }
    ...
    public void doSidebarUpdateSoon() {
        if (updateTask == null) {
            ...
            updateTask = new ScoreboardQuickUpdate().runTaskLater(FactionsCaribe.getInstance(), 2L);
        }
    }
    ...
    private void startCooldownUpdating() {
        if (cooldownTask == null) {
            ...
            cooldownTask = new ScoreboardCooldownTask().runTaskTimer(FactionsCaribe.getInstance(), 5 * Misc.TICK_CONVERSION_FACTOR, 5 * Misc.TICK_CONVERSION_FACTOR);
        }
    }
    ...
    public void setOldScoreboard() {
        ...
        if (oldBoard == scoreboard) { // Already displaying it
            if (this.oldBoard == null) {
                ...
                this.oldBoard = FactionsCaribe.getInstance().getServer().getScoreboardManager().getMainScoreboard();
            }
        } else {
    ...
    public void showBoardAndScheduleRevert(int ticks) {
        ...
        player.setScoreboard(scoreboard);
        revertTask = new ScoreboardChangeTask().runTaskLater(FactionsCaribe.getInstance(), ticks);
